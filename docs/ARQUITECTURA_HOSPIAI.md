...
| **4 · Dominio D2** | Inteligencia clínica | `lector-ocr` (reutilizando la lectura de documentos del Motor de Glosas) escribiendo hallazgos con confianza < 1.0 sobre el Motor de Evidencias, `coherencia-clinica`, `identidad-paciente` |
| **5 · Dominios D4/D6** | Cartera y gerencia | `gestor-devoluciones` + histórico → grafo activo → `recomendador`, `alertas-preventivas` (plazos del catálogo) y agente conversacional; con histórico suficiente, predicción de glosas |
| **6 · Portales** | Radicación automática | `radicador-portales` con human-in-the-loop — **solo** tras autorización de gerencia/TI y análisis legal |
