...
class GlosaService:
    ...
    async def analizar(self, data: GlosaInput, contexto_pdf: str = "", contratos_db: dict = None) -> GlosaResult:
        ...
        # 🏛️ BASE LEGAL INSTITUCIONAL (Resoluciones 2026)
        BASE_LEGAL_HUS = """
        - RESOLUCIÓN INSTITUCIONAL 054 DE 2026: Unifica tarifas de la E.S.E. HUS. Es de OBLIGATORIO CUMPLIMIENTO (Art. 2).
        ...
        # ── CÁLCULO DE TIEMPOS EXTEMPORANEIDAD ──
        ...
        # ══════════════════════════════════════════════════════════════════════
        # 🛡️ 3. GUILLOTINAS LEGALES ABSOLUTAS (BYPASS A LA IA)
        # ══════════════════════════════════════════════════════════════════════

        # A) GLOSA RATIFICADA (Búsqueda parcial de la palabra "RATIF" para evitar fallos de espacios)
        if "RATIF" in etapa_segura and val_ac_num <= 0:
            ...
            texto_rat = (
                "ESE HUS NO ACEPTA LA GLOSA RATIFICADA. SE MANTIENE EN SU INTEGRIDAD LA RESPUESTA DE DEFENSA TÉCNICA Y NORMATIVA "
                "PRESENTADA EN EL TRÁMITE DE LA GLOSA INICIAL, TODA VEZ QUE LA ENTIDAD GLOSANTE NO APORTA NUEVOS ELEMENTOS DE JUICIO "
                "QUE DESVIRTÚEN LA FACTURACIÓN NI LA PRESTACIÓN DEL SERVICIO.\n\n"
                "SE SOLICITA LA PROGRAMACIÓN DE LA FECHA DE CONCILIACIÓN DE AUDITORÍA MÉDICA ENTRE LAS PARTES (CORREO: CARTERA@HUS.GOV.CO). "
                "NOTA: DE ACUERDO CON EL ARTÍCULO 57 DE LA LEY 1438 DE 2011, DE NO LLEGARSE A UN ACUERDO EN LA INSTANCIA DE CONCILIACIÓN, "
                "SE CONTINUARÁ CON LAS ACCIONES DE COBRO PERTINENTES EN LOS TÉRMINOS LEGALES VIGENTES."
            )
            return GlosaResult(
                tipo="LEGAL - RATIFICACIÓN", resumen="RECHAZO DE RATIFICACIÓN",
                dictamen=tabla + _div(texto_rat),
        ...
        if es_extemporanea and val_ac_num <= 0:
            ...
            texto_ext = (
                f"ESE HUS NO ACEPTA LA GLOSA POR EXTEMPORANEIDAD. AL HABER TRANSCURRIDO {dias} DÍAS HÁBILES ENTRE LA FECHA DE RADICACIÓN "
                f"Y LA RECEPCIÓN DE LA GLOSA, SUPERANDO EL TÉRMINO MÁXIMO PERENTORIO E IMPRORROGABLE DE VEINTE (20) DÍAS HÁBILES ESTABLECIDO EN EL ART. 57 DE LA LEY 1438 DE 2011, "
                f"HA OPERADO DE PLENO DERECHO EL FENÓMENO JURÍDICO DE LA ACEPTACIÓN TÁCITA DE LA FACTURA. SE EXIGE EL PAGO INMEDIATO E ÍNTEGRO."
            )
            return GlosaResult(
                tipo="LEGAL - EXTEMPORÁNEA", resumen="RECHAZO POR EXTEMPORANEIDAD",
                dictamen=tabla + _div(texto_ext),
                codigo_glosa=codigo_detectado, valor_objetado=valor_obj_raw,
                paciente="N/A", mensaje_tiempo=msg_tiempo, color_tiempo=color_tiempo
            )

        # ══════════════════════════════════════════════════════════════════════
        # 🧠 4. CEREBRO ADAPTATIVO (LLAMADA A IA)
        # ══════════════════════════════════════════════════════════════════════
        if val_ac_num > 0:
            estrategia = "CASO ACEPTACIÓN: Redacta formalmente que la ESE HUS acepta la glosa por motivos administrativos/pertinencia."
        elif prefijo == "TA":
            estrategia = """ESTRATEGIA TARIFARIA DE MÁXIMA AUTORIDAD (MÍNIMO 3 PÁRRAFOS):
            - PÁRRAFO 1: Cita la RESOLUCIÓN 054 DE 2026 y 120 DE 2026. Explica que unifican las tarifas de la E.S.E. HUS y son OBLIGATORIAS.
            - PÁRRAFO 2: Defiende el cobro a TARIFA SOAT PLENO (100%). Argumenta que la EPS intenta imponer tarifas unilateralmente vulnerando la autonomía administrativa.
            - PÁRRAFO 3: Exige el pago del VALOR OBJETADO invocando el Art. 871 del Código de Comercio (Buena fe). Finaliza indicando CÓDIGO RE9602.
            - REGLA: Prohibido decir 'valor facturado'. Usa SIEMPRE 'VALOR OBJETADO'."""
        elif prefijo == "SO":
            estrategia = "ESTRATEGIA SOPORTES: Localiza en los anexos el documento clínico. Cita médico, RM y fecha. Invoca la Res. 1995/1999 (Historia Clínica es plena prueba). Código RE9602."
        elif prefijo == "FA":
            estrategia = "ESTRATEGIA FACTURACIÓN: Defiende que el servicio es autónomo. Cita Anexo 3 Res 3047. Exige la norma técnica de inclusión a la EPS."
        else:
            estrategia = "ESTRATEGIA INTEGRAL: Defiende la pertinencia médica y el derecho a la salud (Ley 1751), contrastando soportes con el Manual Institucional."

        system_prompt = f"""Eres el DIRECTOR DE JURÍDICA Y AUDITORÍA DE LA ESE HUS (30 años de experiencia).
        REGLAS:
        1. TODO EN MAYÚSCULAS.
        2. PROHIBIDO LAS RESPUESTAS CORTAS: Debes redactar un argumento técnico, profundo y legal (Mínimo 2 párrafos).
        3. USA LA BASE LEGAL DE LA ESE HUS: {BASE_LEGAL_HUS}
        4. LÉXICO: Sinalagma contractual, Realidad fáctica, Preclusión.
        5. NUNCA DIGAS 'valor facturado'. USA SIEMPRE 'VALOR OBJETADO'.
        6. DEVUELVE TU RESPUESTA ESTRICTAMENTE EN ESTE FORMATO XML (Es vital usar las etiquetas):
        ...
        for intento in range(3):
            try:
                completion = await self.cliente.chat.completions.create(
                    ...
                    # 🔥 AQUÍ ESTÁ INTEGRADO LLAMA 4 SCOUT 🔥
                ...
                if "429" in str(e) and intento == 2:
                    res_ia = "<argumento>EL SERVIDOR DE IA HA SUPERADO EL LÍMITE DE VELOCIDAD (ERROR 429). POR FAVOR, ESPERE 40 SEGUNDOS Y VUELVA A INTENTARLO.</argumento>"
                await asyncio.sleep(20)

        # 🛡️ EXTRACCIÓN A PRUEBA DE FALLOS 🛡️
        ...
        # Salvavidas: si la IA olvida la etiqueta XML, atrapamos todo el texto devuelto
        if not argumento_ia:
            argumento_ia = re.sub(r'<[^>]+>', '', res_ia).strip()
            if not argumento_ia:
                argumento_ia = "ERROR AL PROCESAR EL ANÁLISIS. POR FAVOR REINTENTE."
        
        argumento_ia = re.sub(r'[ \t]+', ' ', argumento_ia)

        # ── ENSAMBLAJE DE LA RESPUESTA FINAL ──
        if val_ac_num > 0:
        ...
        return GlosaResult(
            tipo=tipo_final, resumen=res_final, 
            dictamen=tabla_html + f'<div style="text-align:justify;line-height:1.8;font-size:11px;">{dictamen_final.replace("\n", "<br/>")}</div>', 
            codigo_glosa=codigo_final, valor_objetado=valor_xml, 
            paciente=paciente, mensaje_tiempo=msg_tiempo, color_tiempo=color_tiempo
        )
