...
def _disparar_eventos(campo, *eventos: str) -> None:
    ...
    campo.evaluate(
        "(el, evs) => { for (const t of evs) el.dispatchEvent(new Event(t, {bubbles:true})); }",
        list(eventos),
    )
...
def _pestanas_visibles(page: Page) -> bool:
    ...
    return (
        _primer_visible(page.get_by_text("RATIFICADAS", exact=False)) is not None
...
def _abrir_facturas_prestador(page: Page) -> None:
    ...
    while time.time() < deadline:
        caja = _primer_visible(
            page.get_by_text("Facturas prestador", exact=False)
...
def _navegar_por_menu(page: Page) -> None:
    ...
    try:
        cm = _primer_visible(page.locator("text=/Cuentas m[eé]dicas/i")) or _primer_visible(
            page.get_by_text("Cuentas medicas", exact=False)
        )
        if cm is not None:
            cm.click()
            page.wait_for_timeout(800)
        aud = _primer_visible(page.locator("text=/Auditor[ií]a/i")) or _primer_visible(
            page.get_by_text("Auditoria", exact=False)
        )
        if aud is not None:
            aud.click()
...
def _ir_a_auditoria(page: Page) -> None:
    ...
    try:
        ...
        pass
    raise RuntimeError(
        f"No cargó la vista de Auditoría de facturas (URL {page.url}). Texto visible: {cuerpo!r}"
    )
...
def ir_a_pestana(page: Page, tab_label: str) -> None:
    ...
    try:
        page.wait_for_selector("table tbody tr, button:has-text('RESPUESTA'), input", timeout=15000)
    except PlaywrightTimeout:
...
def _dump_inputs(page: Page) -> None:
    ...
    try:
        ...
        for d in data:
            logger.info(
                f"    <{d['tag']}> type={d['type']!r} name={d['name']!r} "
...
def _boton_respuesta_de_fila(page: Page, factura: str):
    ...
    for sel in (
        f"tr:has-text('{factura}')",
        f"[role=row]:has-text('{factura}')",
...
def _elegir_codigo_en_celda(page: Page, celda, codigo: str) -> bool:
    ...
    for intento in range(3):
        ...
        if opcion is None:
            logger.info(
                f"    dropdown Cod Rta2 RAT: no apareció la opción {codigo} "
        ...
        if _click_opcion(opcion):
            return True
        logger.info(
            f"    dropdown Cod Rta2 RAT: clickié {codigo} pero la celda sigue "
...
def _cargar_detalle_en_celda(page: Page, celda, texto: str) -> bool:
    ...
    lapiz = (
        _primer_visible(
            celda.locator("button, [class*=edit i], [class*=pencil i], mat-icon, i, svg, a")
        )
        or celda
    )
    try:
...
def responder_glosa(
    ...
    if not abrir_respuesta(page, factura):
        ...
        reg["detalle"] = (
            "no aparece en la pestaña tras filtrar (¿ya respondida, o no es de esta pestaña?)"
        )
        return reg
    ...
    if i_cod < 0 or i_det < 0:
        ...
        reg["estado"] = "ERROR"
        reg["detalle"] = (
            f"no ubiqué las columnas RTA2 (cod={i_cod}, detalle={i_det}); headers={headers}"
        )
    ...
    if pdf_dir is not None and pdf is None:
        logger.warning(
            f"  ⚠ no encontré {factura}.pdf en {pdf_dir} — la fila se guardará sin archivo."
        )

    logger.info(
        f"  {factura}: {len(filas)} fila(s) de servicio; cod={codigo}"
    ...
    btn_guardar = _primer_visible(
        page.locator("button:has-text('GUARDAR RESPUESTA'), button:has-text('Guardar Respuesta')")
    )
...
def diagnosticar_respuesta(page: Page, factura: str) -> None:
    ...
    logger.info(
        f"    idx Detalle Rta 2     = {_idx_columna(headers, 'DETALLERTA2PRESTADOR', 'DETALLERTA2')}"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--evidencias",
        type=Path,
        default=Path("EVIDENCIA_FOMAG"),
        help="Carpeta para los pantallazos de cierre.",
    )
    parser.add_argument(
        "--reporte",
        type=Path,
        default=Path("reporte_fomag.csv"),
    ...
    if args.solo:
        ...
        objetivos = [
            x.strip().upper() for x in contenido.replace(",", "\n").splitlines() if x.strip()
        ]
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if args.listar:
                ...
                logger.info(
                    f"Respondiendo {len(objetivos)} factura(s) de {tab_label} "
                    f"con código {args.cod}" + (" [SIN GUARDAR]" if args.sin_guardar else "") + "."
                )
                args.reporte.parent.mkdir(parents=True, exist_ok=True)
                with args.reporte.open("w", newline="", encoding="utf-8-sig") as f_rep:
                    w_rep = csv.DictWriter(
                        f_rep, fieldnames=["factura", "filas", "ok", "estado", "detalle"]
                    )
                    w_rep.writeheader()
                    for i, fac in enumerate(objetivos, start=1):
