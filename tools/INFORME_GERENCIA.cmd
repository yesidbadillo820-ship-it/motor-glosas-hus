@echo off
REM ====================================================================
REM  INFORME_GERENCIA.cmd  -  Bot de doble clic para el Motor Glosas HUS.
REM  Genera el informe de gestion para gerencia a partir de la bitacora
REM  REGISTRO_BOTS.csv que llena el menu MOTOR_HUS: usos por bot, por
REM  persona y por semana, en un HTML listo para imprimir o enviar.
REM  Se abre solo al terminar.
REM
REM  Se instala solo Python si falta. USO: doble clic.
REM ====================================================================
chcp 65001 >nul 2>&1
setlocal EnableExtensions DisableDelayedExpansion
title INFORME GERENCIA - uso de los bots - Motor Glosas HUS

echo.
echo ============================================================
echo   INFORME GERENCIA - uso de los bots
echo ============================================================
echo.
...
set "MOTOR=%~dp0informe_gerencia.py"
if exist "%MOTOR%" goto pedir
set "MOTOR=%~dp0tools\informe_gerencia.py"
if exist "%MOTOR%" goto pedir
set "MOTOR=%TEMP%\informe_gerencia_hus.py"
set "ORIGENCMD=%~f0"
del "%MOTOR%" >nul 2>&1
...
:pedir
REM --- 4) La bitacora debe existir junto a este .cmd ------------------
if not exist "%~dp0REGISTRO_BOTS.csv" (
  echo [ERROR] No hay REGISTRO_BOTS.csv junto a este .cmd.
  echo         Usa los bots desde MOTOR_HUS.cmd - el menu crea la bitacora solo -
  echo         y vuelve a generar el informe.
  echo.
  pause
  exit /b 2
)

REM --- 5) Ejecutar y abrir el informe ----------------------------------
%PYEXE% "%MOTOR%" "%~dp0REGISTRO_BOTS.csv" --salida "%~dp0INFORME_GERENCIA.html"
if not errorlevel 1 start "" "%~dp0INFORME_GERENCIA.html"
...
:sinmotor
echo [ERROR] No se pudo preparar el motor. Copia tambien "informe_gerencia.py"
...
REM ====================================================================
REM  Motor Python embebido (copia de tools\informe_gerencia.py).
