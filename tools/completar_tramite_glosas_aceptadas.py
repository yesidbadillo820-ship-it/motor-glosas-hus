...
def a_numero(v):
    ...
    Lo que hacía antes: `float(str(v).replace(",", ""))`. Solo quitaba comas,
    así que el formato colombiano lo destruía:

        "1.234.567"  →  ValueError  →  0        (un millón se volvía cero)
        "$114.900"   →  ValueError  →  0
        "50.000"     →  50                      (dividido por mil)
        "2.500"      →  2                       (dividido por mil)
        "1.365,50"   →  1                       (dividido por mil)

    Esta herramienta completa el acta de las glosas aceptadas: el valor que
    escribía era cero o la milésima parte del real.
    """
    return a_entero(v)
