...
class ExcelExporter:
    ...
    def _agregar_encabezado(
        self, ws, titulo: str, fecha_inicio: Optional[str], fecha_fin: Optional[str]
    ):
        ...
        for row_idx, glosa in enumerate(glosas, 6):
            ...
            for col, valor in enumerate(valores, 1):
                ...
                cell.alignment = Alignment(
                    horizontal="center" if col in [1, 7, 10, 11, 12] else "left"
                )
    ...
    def _agregar_totales(self, ws, glosas: list):
        ...
        ws.cell(
            row=last_row + 1,
            column=11,
            value=f"Recuperado: {total_acept / total_obj * 100:.1f}%" if total_obj > 0 else "N/A",
        ).font = Font(bold=True, color="16a34a")
    ...
    def generar_resumen_mensual(
        ...
        for row_idx, t in enumerate(tendencias, 4):
            ...
            ws.cell(row=row_idx, column=6).font = Font(
                color="16a34a" if pct > 70 else "dc2626", bold=True
            )
