...
class PlantillaRepository:
    ...
    def crear(
        self, nombre: str, codigo: str, tipo: str, eps: str, plantilla: str
    ) -> PlantillaRecord:
    ...
    def obtener_por_codigo(self, codigo: str) -> Optional[PlantillaRecord]:
        return (
            self.db.query(PlantillaRecord)
            .filter(PlantillaRecord.codigo == codigo.upper(), PlantillaRecord.activa == 1)
            .first()
        )

    def actualizar(
        self, id: int, nombre: str = None, plantilla: str = None, activa: int = None
    ) -> Optional[PlantillaRecord]:
        record = self.obtener_por_id(id)
        if record:
            if nombre:
                record.nombre = nombre
            if plantilla:
                record.plantilla = plantilla
            if activa is not None:
                record.activa = activa
            self.db.commit()
            self.db.refresh(record)
        return record
    ...
    def get_plantilla_eps(self, eps: str, tipo: str) -> Optional[str]:
        ...
        rec = (
            self.db.query(PlantillaRecord)
            .filter(
                PlantillaRecord.eps == "", PlantillaRecord.tipo == tipo, PlantillaRecord.activa == 1
            )
            .first()
        )
        if rec:
        ...
        rec = (
            self.db.query(PlantillaRecord)
            .filter(PlantillaRecord.tipo == "FALLBACK", PlantillaRecord.activa == 1)
            .first()
        )
