...
async def noticias_recientes(
    ...
    total_existente = db.query(NoticiaSaludRecord).filter(NoticiaSaludRecord.activa == 1).count()
...
def noticias_stats(
    ...
    activas = (
        db.query(func.count(NoticiaSaludRecord.id)).filter(NoticiaSaludRecord.activa == 1).scalar()
        or 0
    )
    por_fuente = dict(
