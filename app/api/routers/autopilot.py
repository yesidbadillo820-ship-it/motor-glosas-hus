...
def evaluar_glosa(
    ...
    ya_cerrada = wf_actual in ("RESPONDIDA", "CONCILIADA", "LEVANTADA") or estado_actual in (
        "RESPONDIDA",
        "CONCILIADA",
        "LEVANTADA",
    )
        ...
        try:
            ...
            if not (g.nota_workflow or "").strip():
                ...
                tipo = (
                    "RATIFICADA"
                    if "ratificada" in m
                    else ("EXTEMPORANEA" if "extemporanea" in m else "texto_fijo")
                )
