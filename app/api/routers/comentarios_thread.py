...
def listar_por_glosa(
    ...
    q = db.query(ComentarioThreadRecord).filter(ComentarioThreadRecord.glosa_id == glosa_id)
...
def resolver(
    ...
    c = db.query(ComentarioThreadRecord).filter(ComentarioThreadRecord.id == comentario_id).first()
    if not c:
