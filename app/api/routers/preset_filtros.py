...
def listar_presets(
    ...
    rows = (
        ...
        .order_by(PresetFiltroRecord.uso_count.desc(), PresetFiltroRecord.creado_en.desc())
        .all()
    )
...
def crear_preset(
    ...
    if data.visibilidad == "GLOBAL" and (current_user.rol or "").upper() not in (
        "COORDINADOR",
        "SUPER_ADMIN",
    ):
    ...
    count = (
        db.query(PresetFiltroRecord)
        .filter(PresetFiltroRecord.usuario_email == current_user.email)
        .count()
    )
    if count >= 30:
