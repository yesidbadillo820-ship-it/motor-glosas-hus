...
def _calcular_sugerencia(db: Session, glosa_id: int) -> dict:
    ...
    usuarios = (
        ...
        .filter(
            ...
            UsuarioRecord.rol.in_(["AUDITOR", "COORDINADOR", "SUPER_ADMIN"]),
        )
        .all()
    )
    ...
    for u in usuarios:
        ...
        exitosas = sum(
            1
            for g in historicas
            if (g.valor_objetado or 0) > 0
            and ((g.valor_objetado or 0) - (g.valor_aceptado or 0)) / (g.valor_objetado or 1) > 0.7
        )
