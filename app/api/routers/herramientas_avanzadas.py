...
class SimuladorRequest(BaseModel):
    ...
    eps: str = ""
    escenario: str = Field(
        ..., description="'con_contrato' | 'con_pdf' | 'extemporanea' | 'ratificacion'"
    )
...
def learning_mis_patrones(
    ...
    try:
        versiones = (
            ...
            .filter(
                VersionRecord.created_at >= desde if hasattr(VersionRecord, "created_at") else True
            )
            .all()
        )
    except Exception:
    ...
    if len(versiones) >= 3:
        ...
        cuenta_acortar = sum(
            1 for m in motivos if "CORT" in m or "RESUM" in m or "MENOS PALABRAS" in m
        )
        cuenta_citar = sum(1 for m in motivos if "CIT" in m or "NORMA" in m or "ART" in m)
        cuenta_tono = sum(
            1 for m in motivos if "TONO" in m or "SUAV" in m or "FIRM" in m or "CONCILIADOR" in m
        )
        cuenta_ampliar = sum(
            1 for m in motivos if "AMPLIA" in m or "EXTIENDE" in m or "MAS DETAL" in m
        )
        cuenta_clinico = sum(
            1 for m in motivos if "CLINIC" in m or "HC" in m or "HISTORIA CLIN" in m or "FOLIO" in m
        )
