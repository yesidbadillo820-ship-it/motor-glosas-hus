...
def learning_mis_patrones(
    ...
    if len(versiones) >= 3:
        ...
        motivos = [str(v.notas or v.motivo or "").upper() for v in versiones if hasattr(v, "notas") or hasattr(v, "motivo")]
        if not motivos:
            motivos = []
        cuenta_acortar = sum(1 for m in motivos if "CORT" in m or "RESUM" in m)
        cuenta_citar = sum(1 for m in motivos if "CIT" in m or "NORMA" in m or "ART" in m)
        cuenta_tono = sum(1 for m in motivos if "TONO" in m or "SUAV" in m or "FIRM" in m)
        if cuenta_acortar > 1:
            patrones["tendencias"].append({"patron": "Acortar respuestas", "frecuencia": cuenta_acortar})
        if cuenta_citar > 1:
            patrones["tendencias"].append({"patron": "Agregar citas normativas", "frecuencia": cuenta_citar})
        if cuenta_tono > 1:
            patrones["tendencias"].append({"patron": "Ajustar tono", "frecuencia": cuenta_tono})
