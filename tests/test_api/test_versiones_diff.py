...
class TestDiffVersiones:
    def test_diff_basico(self, client, db_session, glosa):
        ...
        v2 = _seed_version(
            db_session, glosa.id, "<p>linea uno</p><p>linea TRES</p>", accion="REANALIZAR"
        )
        r = client.get(f"/glosas/{glosa.id}/versiones/diff?v1={v1.id}&v2={v2.id}")
    ...
    def test_diff_v1_igual_v2_devuelve_400(self, client, db_session, glosa):
        ...
        r = client.get(f"/glosas/{glosa.id}/versiones/diff?v1={v.id}&v2={v.id}")
        assert r.status_code == 400

    def test_diff_version_inexistente_404(self, client, db_session, glosa):
        v = _seed_version(db_session, glosa.id, "<p>x</p>")
        r = client.get(f"/glosas/{glosa.id}/versiones/diff?v1={v.id}&v2=99999")
