...
class TestCumplimientoSLA:
    ...
    def test_clasifica_por_dias_restantes(self, client, db_session):
        ...
        _seed(db_session, dias_restantes=-3, valor_objetado=10_000, estado="RADICADA")
        # Crítica (entre 0 y 3, abierta)
        _seed(db_session, dias_restantes=2, valor_objetado=20_000, estado="RADICADA")
        # En tiempo (>3 días, abierta)
        _seed(db_session, dias_restantes=10, valor_objetado=30_000, estado="RADICADA")
    ...
    def test_cerradas_no_cuentan_como_vencidas(self, client, db_session):
        ...
        _seed(db_session, dias_restantes=-100, estado="ACEPTADA", valor_objetado=999)
        r = client.get("/glosas/stats/cumplimiento-sla")
        d = r.json()
        assert d["vencidas"] == 0
        assert d["cerradas"] == 1
