...
class TestPorDiaSemana:
    ...
    def test_clasifica_por_dia(self, client, db_session):
        # Anclar a un lunes reciente DENTRO de la ventana (default 90 días):
        # con fechas fijas de abril el test caducaba al pasar 90 días de ellas.
        hoy = ahora_utc()
        lunes = (hoy - timedelta(days=hoy.weekday() + 7)).replace(
            hour=12, minute=0, second=0, microsecond=0
        )
        miercoles = lunes + timedelta(days=2)
        _seed(db_session, lunes)
        _seed(db_session, lunes.replace(hour=13))
