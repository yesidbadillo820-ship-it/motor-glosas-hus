...
class TestPerformanceHistorica:
    def test_estructura(self, client):
        ...
        d = r.json()
        for key in ("usuario_email", "ventana_meses", "total_meses_con_actividad", "serie"):
            assert key in d

    def test_solo_glosas_propias(self, client, db_session):
        _seed(db_session, "Alice", datetime(2026, 4, 5, tzinfo=timezone.utc))
        _seed(db_session, "Bob", datetime(2026, 4, 5, tzinfo=timezone.utc))
    ...
    def test_agrupa_por_mes(self, client, db_session):
        _seed(db_session, "Alice", datetime(2026, 4, 5, tzinfo=timezone.utc))
        _seed(db_session, "Alice", datetime(2026, 4, 20, tzinfo=timezone.utc))
        _seed(db_session, "Alice", datetime(2026, 3, 1, tzinfo=timezone.utc))
    ...
    def test_tasa_levantamiento(self, client, db_session):
        ...
        _seed(db_session, "Alice", datetime(2026, 4, 1, tzinfo=timezone.utc), estado="LEVANTADA")
        _seed(db_session, "Alice", datetime(2026, 4, 5, tzinfo=timezone.utc), estado="LEVANTADA")
        _seed(db_session, "Alice", datetime(2026, 4, 10, tzinfo=timezone.utc), estado="ACEPTADA")
        r = client.get("/usuarios/yo/performance-historica?meses=24")
        d = r.json()
    ...
    def test_serie_ascendente(self, client, db_session):
        for mes in (1, 4, 3):
            _seed(db_session, "Alice", datetime(2026, mes, 5, tzinfo=timezone.utc))
        r = client.get("/usuarios/yo/performance-historica?meses=24")
        d = r.json()
