...
class TestHistorialReasignaciones:
    def test_estructura(self, client):
        ...
        d = r.json()
        for key in ("ventana_dias", "total_reasignaciones", "top_5_quien_reasigna", "items"):
            assert key in d
    ...
    def test_filtra_solo_campos_relevantes(self, client, db_session):
        ...
        _seed_audit(db_session, "u@x", "auditor_email", "old@x", "new@x")
    ...
    def test_top_5_quien_reasigna(self, client, db_session):
        for _ in range(5):
            _seed_audit(db_session, "alice@x", "gestor_nombre", "X", "Y")
        for _ in range(2):
            _seed_audit(db_session, "bob@x", "gestor_nombre", "X", "Y")
    ...
    def test_excluye_fuera_de_ventana(self, client, db_session):
        _seed_audit(db_session, "u@x", "gestor_nombre", "A", "B", dias_atras=5)
        _seed_audit(db_session, "u@x", "gestor_nombre", "A", "B", dias_atras=60)
