...
class TestValorEficienciaAnual:
    ...
    def test_eficiencia_anual(self, client, db_session):
        _seed(db_session, datetime(2025, 6, 1, tzinfo=timezone.utc), obj=10_000_000, rec=5_000_000)
        _seed(db_session, datetime(2026, 1, 1, tzinfo=timezone.utc), obj=4_000_000, rec=3_000_000)
    ...
    def test_serie_ascendente(self, client, db_session):
        _seed(db_session, datetime(2026, 1, 1, tzinfo=timezone.utc), obj=100, rec=0)
        _seed(db_session, datetime(2024, 1, 1, tzinfo=timezone.utc), obj=100, rec=0)
        r = client.get("/glosas/stats/valor-eficiencia-anual")
        d = r.json()
