...
class TestPapeleraBuscar:
    def test_estructura(self, client):
        ...
        d = r.json()
        for key in ("filtro_glosa_id_original", "filtro_eliminado_por", "total", "items"):
            assert key in d
    ...
    def test_combina_filtros(self, client, db_session):
        ...
        r = client.get("/papelera/buscar?glosa_id_original=1&eliminado_por=alice@x")
        d = r.json()
        assert d["total"] == 1
