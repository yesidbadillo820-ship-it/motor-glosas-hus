...
class TestEPSActividadMensual:
    def test_estructura(self, client):
        r = client.get("/glosas/stats/eps-actividad-mensual?eps=SANITAS")
        assert r.status_code == 200, r.text
        d = r.json()
        for key in ("eps", "meses_solicitados", "total_meses_disponibles", "serie"):
            assert key in d
    ...
    def test_serie_basica(self, client, db_session):
        ...
        _seed(db_session, "SANITAS", datetime(2026, 4, 20, tzinfo=timezone.utc), valor_obj=20_000)
        _seed(db_session, "SANITAS", datetime(2026, 3, 1, tzinfo=timezone.utc), valor_obj=5_000)

        r = client.get("/glosas/stats/eps-actividad-mensual?eps=SANITAS&meses=24")
        d = r.json()
    ...
    def test_aislamiento_por_eps(self, client, db_session):
        _seed(db_session, "SANITAS", datetime(2026, 4, 5, tzinfo=timezone.utc))
        _seed(db_session, "OTRA", datetime(2026, 4, 5, tzinfo=timezone.utc))
