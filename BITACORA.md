...
- **FASE 3 — OPERATIONAL INTELLIGENCE (aprobada la 2.2, cambia el eje):** por directiva, la prioridad deja de ser construir agentes y pasa a ser **subir las facturas LISTAS y bajar los tiempos**. Todo desarrollo nuevo debe demostrar impacto operativo:
  - **Regla de oro (obligatoria):** antes de crear cualquier agente hay que responder 5 preguntas — qué problema resuelve, cuánto dinero libera, cuántas horas ahorra, qué indicador mejora y cómo se medirá antes/después. Las respuestas quedaron escritas en la ficha de cada agente nuevo (`justificacion_operativa` en el registro) y **hay una prueba automática que las exige**.
  - **AG024 Plan de Recuperación** — criterio de aceptación cumplido: `py tools\hospiai.py plan HUS528043` responde el plan completo: qué hacer, **quién** (área curada en `data/esfuerzos.json`), **cuánto tarda** (estimación del área, editable — jamás inventada), **probabilidad** (por regla vigente o historia validada) e **impacto** ("pasa inmediatamente a LISTA y libera $X"). Y lo más potente: si el soporte que falta **YA existe en un servidor indexado**, el plan lo dice ("Asociar HEV: ya existe en el servidor Y") — la próxima corrida lo anexa sola.
  - **AG025 Corrección Masiva** — criterio de aceptación cumplido: `py tools\hospiai.py oportunidades` encuentra soluciones por miles: soportes que ya existen y solo hay que asociar, faltantes únicos por conseguir, archivos sin tipificar (potencial, sin dinero inventado) y entidades sin catálogo — cada una con **dinero exacto por reglas** y el TOTAL recuperable. **Solo propone: jamás ejecuta ni mueve archivos.**
  - **AG026 Inteligencia de Contratos** (`py tools\hospiai_operacion.py ficha`): la ficha viva de cada pagador — % listas, valor detenido, causas frecuentes, plazo, glosas. Sin historial cargado lo dice, no lo inventa.
  - **AG027 Gemelo Digital de EPS** (`gemelo NUEVA`): cómo se comporta cada EPS, calculado de los datos (exigencia observada, faltantes típicos, lecciones validadas); la demora promedio y lo que devuelve quedan pendientes del historial de glosas/pagos — declarado, no inventado.
  - **AG028 Mejora Continua** (`mejora`): el ritual del viernes — qué mejoró, qué empeoró, qué aprendimos, qué cambiamos el lunes — comparando las dos últimas corridas. La corrida diaria lo corre sola los viernes.
  - **Indicadores obligatorios por corrida** (tabla `indicadores` + `py tools\hospiai_operacion.py indicadores`): financieros (listo/detenido/recuperable/perdido), operativos (segundos de auditoría por factura, días hasta LISTA, radicación y pago), calidad (% por dictamen) y aprendizaje (lecciones, patrones, recomendaciones aceptadas/exitosas). **Los que aún no se pueden medir quedan vacíos con su razón** — el "valor perdido" espera los plazos por contrato y los tiempos de pago esperan el historial.
